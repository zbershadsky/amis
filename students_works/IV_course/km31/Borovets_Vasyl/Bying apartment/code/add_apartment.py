...
from check import *
from seller_function import add_apartments
from PyQt5.QtWidgets import QMessageBox

class Ui_add_apartment(object):
    def setupUi(self, add_apartment):
        ...
        self.ok_but.clicked.connect(self.adding_flat)

        self.retranslateUi(add_apartment)
        QtCore.QMetaObject.connectSlotsByName(add_apartment)
    ...
    def adding_flat(self):
        ...
        if (check_title(self.title_ed.text())==False):
            ...
            try:
                add_apartments(self.title_ed.text(), self.login, self.title_ed_2.text(),self.city_ed.text(),self.street_ed.text(),
                       self.home_ed.text(), self.flat_ed.text(),self.price_ed.text())
                self.win.close()
            except:
                QMessageBox.information(self.win, 'Error', "DataBase error")
    ...
    def retranslateUi(self, add_apartment):
        ...
        self.label_6.setText(_translate("add_apartment", "ADD APARTMENT"))
